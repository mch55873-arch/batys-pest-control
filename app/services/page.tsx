...
export default function ServicesPage() {
  ...
  const installationStyle = pestServices
    .filter((service) => /inspection|prevention|proof|exclusion|monitor/i.test(`${service.name} ${service.slug}`))
    .slice(0, 10);
  const treatmentStyle = pestServices
    .filter((service) => !installationStyle.includes(service))
    .slice(0, 12);
    ...
    <>
      <section className="bg-[#0b1c2d] px-4 py-20 text-white sm:py-24">
        <div className="mx-auto max-w-7xl">
          <nav className="text-sm font-semibold text-slate-300">
            <Link href="/">Home</Link><span className="mx-2">/</span>Services
          </nav>
          <h1 className="mt-8 max-w-4xl font-heading text-5xl font-black leading-[1.05] tracking-tight sm:text-6xl">
            Complete Pest Control Services <span className="text-[#16b7df]">Across the USA</span>
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-200">
            Choose a pest, inspection need, treatment method, or property type. Every service page explains the warning signs, expected process, preparation, prevention, and questions to ask before hiring an independent provider.
          </p>
        </div>
      </section>
      ...
      <section className="bg-[#f4f7f9] px-4 py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-3 max-w-3xl leading-7 text-slate-600">
            Dedicated service pages are linked below. Not sure which service fits the problem? Call and describe the pest signs, affected areas, property type, and urgency.
          </p>
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-2">
            <section className="rounded-2xl border border-slate-200 bg-white p-7 sm:p-8">
              ...
              <div className="mt-6 grid gap-x-7 gap-y-3 sm:grid-cols-2">
                {installationStyle.map((service) => (
                  <Link key={service.slug} href={`/services/${service.slug}`} className="text-sm font-bold leading-6 text-slate-600 hover:text-sky-600">
                    <span className="mr-2 text-sky-500">✓</span>{service.name}
                  </Link>
                ))}
              </div>
      ...
      <section className="px-4 py-16 text-center">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mx-auto mt-3 max-w-2xl leading-7 text-slate-600">
            Research common household pests, wood-destroying insects, rodents, wildlife, mosquitoes, stored-product pests, and commercial facility concerns.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-x-10 gap-y-4 text-lg font-black text-slate-500">
            {['Termites', 'Bed Bugs', 'Rodents', 'Cockroaches', 'Ants', 'Mosquitoes', 'Wildlife'].map((item) => <span key={item}>{item}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#f4f7f9] px-4 py-16 text-center">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mx-auto mt-3 max-w-2xl leading-7 text-slate-600">
            Every service above connects to state and city routes so visitors can keep the correct geographic context while reviewing pest-control options.
          </p>
          <div className="mt-7 flex flex-wrap justify-center gap-3">
            {['California', 'Texas', 'Florida', 'New York', 'Pennsylvania', 'Ohio'].map((state) => (
              <span key={state} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700">{state}</span>
            ))}
            <Link href="/locations" className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-black text-sky-600">All service areas →</Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#16b7df] px-4 py-12 text-white">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-6 md:flex-row md:items-center">
          <div>
            <h2 className="font-heading text-3xl font-black">Not Sure Which Service You Need?</h2>
            <p className="mt-2 text-sky-50">Call and describe what is happening. Provider availability, licensing, methods, and pricing must be confirmed directly.</p>
          </div>
