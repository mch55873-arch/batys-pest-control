...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Browse pest-control services, cities, inspections, treatments, removal, and prevention information across ${state.name}.`,
    alternates: { canonical: stateUrl(state) },
    openGraph: {
      title: `Pest Control Services in ${state.name}`,
      description: `Choose a city and review local pest-control service information throughout ${state.name}.`,
      url: stateUrl(state),
      images: [{ url: '/images/pest-control-hero.webp', alt: `Pest-control services across ${state.name}` }],
    },
  };
}
...
export default async function StatePage({ params }: Props) {
  ...
  const faqs = [
    [`How do I find pest control in ${state.name}?`, `Choose a city below, select the pest or service needed, and confirm provider coverage, licensing, insurance, inspection scope, methods, pricing, and availability directly.`],
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden bg-[#0b1c2d] px-4 py-20 text-white sm:py-24">
        <Image src="/images/pest-control-hero.webp" alt={`Pest-control service information for ${state.name}`} fill priority unoptimized className="object-cover opacity-30" sizes="100vw" />
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <span className="mt-8 inline-flex rounded-full bg-emerald-500/15 px-4 py-2 text-xs font-black uppercase tracking-wider text-emerald-200">Statewide pest-control directory</span>
          ...
          <div className="mt-8 flex flex-wrap gap-4"><a href={SITE.phoneHref} className="rounded-xl bg-[#f47b20] px-7 py-4 font-black">Call {SITE.phoneDisplay}</a><a href="#cities" className="rounded-xl border border-white/25 bg-white/10 px-7 py-4 font-black">Choose Your City</a></div>
        </div>
      </section>

      <section className="border-b border-slate-200 bg-white px-4">
        <div className="mx-auto grid max-w-7xl grid-cols-2 md:grid-cols-4">
          {[[cities.length.toLocaleString('en-US'), 'Cities & communities'], [pestServices.length.toString(), 'Service topics'], ['50+', 'Common pest entities'], ['Direct', 'Phone availability check']].map(([value, label], index) => <div key={label} className={`p-6 text-center ${index ? 'border-l border-slate-200' : ''}`}><strong className="font-heading text-3xl font-black text-[#0b1c2d]">{value}</strong><p className="mt-1 text-xs font-bold uppercase tracking-wider text-slate-500">{label}</p></div>)}
        </div>
      </section>
      ...
      <section className="bg-[#f4f7f9] px-4 py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-4 max-w-3xl leading-7 text-slate-600">Start with the pest or service intent, then select a city for locally organized information and provider questions.</p>
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {featured.map((service, index) => <Link key={service.slug} href={`/services/${service.slug}`} className="group rounded-2xl border border-slate-200 bg-white p-7 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><span className="grid h-11 w-11 place-items-center rounded-xl bg-sky-50 font-black text-sky-600">0{index + 1}</span><h3 className="mt-5 font-heading text-xl font-black text-[#0b1c2d] group-hover:text-sky-600">{service.name}</h3><p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600">{service.description}</p><span className="mt-5 inline-flex text-sm font-black text-sky-600">View service →</span></Link>)}
          </div>
        </div>
      </section>
      ...
      <section id="cities" className="px-4 py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <p className="mt-4 max-w-3xl leading-7 text-slate-600">Select a city to browse local pest-control service routes, warning signs, treatment considerations, preparation guidance, and frequently asked questions.</p>
          <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {cities.map((city) => <a key={city.slug} href={cityUrl(state, city)} className="rounded-xl border border-slate-200 bg-white px-4 py-4 text-sm font-bold text-[#0b1c2d] shadow-sm transition hover:border-sky-400 hover:text-sky-600">{city.name}<span className="float-right text-sky-500">→</span></a>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#16b7df] px-4 py-12 text-white"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-6 md:flex-row md:items-center"><div><h2 className="font-heading text-3xl font-black">Need pest-control information in {state.name}?</h2><p className="mt-2 text-sky-50">Choose your city or call to check independent-provider availability.</p></div><a href={SITE.phoneHref} className="rounded-xl bg-[#0b1c2d] px-7 py-4 font-black">Call {SITE.phoneDisplay}</a></div></section>
    </>
  );
}
