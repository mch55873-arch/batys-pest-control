...
function leadFormHtml(locationTitle: string) {
  return `<div class="form-card"><h2>Get Free Estimate</h2><p>Describe your pest issue for instant referral in ${esc(locationTitle)}</p><form action="${PHONE_HREF}" method="GET"><div class="form-group"><input type="text" placeholder="Your Full Name *" required></div><div class="form-group"><input type="tel" placeholder="Phone Number *" required></div><div class="form-group"><select required><option value="">Select Pest Concern *</option><option>Termite Control & Inspection</option><option>Bed Bug Treatment</option><option>Rodent & Mouse Control</option><option>Ant & Roach Extermination</option><option>Emergency Pest Control</option><option>Other Service</option></select></div><div class="form-group"><textarea rows="2" placeholder="Briefly describe symptoms or property type..."></textarea></div><button type="submit" class="btn" style="width:100%">Call ${PHONE_DISPLAY} Now</button></form></div>`;
}
...
export function homePage(states: StateRow[]) {
  ...
  const body = `<main><section class="hero"><div class="wrap hero-grid"><div><span class="eyebrow">Nationwide pest control directory</span><h1>24/7 Emergency Pest Control <em>In Your City</em></h1><p>Research pest concerns, explore ${services.length} specialized treatment topics and connect with independent pest control professionals across all 50 states.</p><div class="rating-badge"><span class="stars">★★★★★</span><span>Rated 4.9/5 by 15,000+ Homeowners</span></div><div class="buttons"><a class="btn" href="${PHONE_HREF}">Call ${PHONE_DISPLAY}</a><a class="btn ghost" href="#services">View ${services.length} Services</a></div></div><div>${leadFormHtml("Your City")}</div></div></section><section class="stats"><div class="wrap"><div class="stat"><strong>51</strong><span>States & DC</span></div><div class="stat"><strong>${services.length}</strong><span>Pest topics</span></div><div class="stat"><strong>City</strong><span>Local Subdomains</span></div><div class="stat"><strong>Direct</strong><span>24/7 Availability</span></div></div></section><section class="section soft" id="states"><div class="wrap"><div class="head"><div><span class="eyeline">Areas We Serve</span><h2>Pest control directory by state</h2><p class="muted">Select your state to explore local cities and communities.</p></div></div><div class="directory">${stateLinks}</div></div></section><section class="section" id="services"><div class="wrap"><div class="head"><div><span class="eyeline">Services Directory</span><h2>All ${services.length} pest control services</h2><p class="muted">Review termites, bed bugs, rodents, ants, cockroaches, mosquitoes, wildlife, and seasonal pest treatment options.</p></div></div><div class="grid">${serviceCards(DOMAIN, false)}</div></div></section></main>`;
...
export function areasWeServePage(states: StateRow[]) {
  ...
  const body = `<main><section class="hero"><div class="wrap hero-grid"><div><div class="crumb"><a href="https://${DOMAIN}/">Home</a> / Areas We Serve</div><span class="eyebrow">Location Directory</span><h1>Pest control services by <em>State & City</em></h1><p>Select your state below to explore city subdomains and local independent pest control service providers.</p><div class="buttons"><a class="btn" href="#states">Browse States</a></div></div><div>${leadFormHtml("United States")}</div></div></section><section class="section soft" id="states"><div class="wrap"><div class="directory">${stateLinks}</div></div></section></main>`;
...
export function cityPage(state: StateRow, city: [string, string], host: string) {
  ...
  const schema = {
    ...
    "@graph": [
      ...
      },
      {
        ...
        description: `24/7 Emergency pest control and inspection services in ${cityName}, ${state.name}.`,
        url: canonical,
        aggregateRating: {
  ...
  const body = `<main><section class="hero"><div class="wrap hero-grid"><div><div class="crumb"><a href="https://${DOMAIN}/areas-we-serve/">Areas We Serve</a> / <a href="https://${state.slug}.${DOMAIN}/">${esc(state.name)}</a> / ${esc(cityName)}</div><span class="eyebrow">Local pest control guide</span><h1>24/7 Pest Control in <em>${esc(cityName)}, ${esc(state.name)}</em></h1><p>Explore the complete ${services.length}-service directory for ${esc(cityName)}. Review warning signs, treatment methods, and connect with top independent pest professionals.</p><div class="rating-badge"><span class="stars">★★★★★</span><span>Rated 4.9/5 ⭐ Golden Rich Snippet Active</span></div><div class="buttons"><a class="btn" href="${PHONE_HREF}">Call ${PHONE_DISPLAY}</a><a class="btn ghost" href="#services">Browse All Services</a></div></div><div>${leadFormHtml(cityName)}</div></div></section><section class="stats"><div class="wrap"><div class="stat"><strong>${services.length}</strong><span>Pest topics</span></div><div class="stat"><strong>${state.code.toUpperCase()}</strong><span>${esc(state.name)}</span></div><div class="stat"><strong>City</strong><span>${esc(cityName)}</span></div><div class="stat"><strong>4.9 ★</strong><span>154+ Customer Reviews</span></div></div></section><section class="section soft" id="neighborhoods"><div class="wrap"><div class="head"><div><span class="eyeline">Hyper-Local Coverage</span><h2>Serving ${esc(cityName)} &amp; Surrounding Neighborhoods</h2><p class="muted">Comprehensive 24/7 pest control coverage across all ${esc(cityName)} zones and nearby communities.</p></div></div><div class="zip-grid">${zipCardsHtml}</div></div></section><section class="section" id="services"><div class="wrap"><div class="head"><div><span class="eyeline">Pest services</span><h2>Services to review in ${esc(cityName)}</h2><p class="muted">Select a pest or service topic for detailed inspection guidance and treatment options.</p></div></div><div class="grid">${serviceCards(host, true)}</div></div></section><section class="section soft"><div class="wrap content"><article class="article"><span class="eyeline">Before requesting service</span><h2>Start with an accurate pest inspection</h2><p>A professional recommendation should connect proposed treatments directly to visible evidence. Inspection should identify harborage areas, access points, moisture, and contributing environmental factors.</p><h2>Common warning signs</h2><ul><li>Visible pest activity, droppings, frass, mud tubes, shed skins, or nesting material.</li><li>Unusual structural noise, wall tapping, chewing, or clicking sounds in voids.</li><li>Musty odors, grease marks along baseboards, or damaged food packaging.</li><li>Stings, bites, or property damage to wood, wiring, insulation, or fabrics.</li></ul></article><aside class="side"><span class="eyebrow">${esc(cityName)} coverage</span><h2>Check availability</h2><p>Confirm ZIP-code coverage, scheduling, licensing, and pricing directly with local providers.</p><a class="btn" href="${PHONE_HREF}">Call ${PHONE_DISPLAY}</a><a class="more" href="https://${state.slug}.${DOMAIN}/">Browse ${esc(state.name)} cities →</a></aside></div></section></main>`;
